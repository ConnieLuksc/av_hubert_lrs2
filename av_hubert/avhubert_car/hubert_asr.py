...
class HubertEncoder(FairseqEncoder):
    def __init__(self, cfg: AVHubertAsrConfig, tgt_dict=None):
        ...
        # Disabled normalizef check (
        #     "Fine-tuning works best when data normalization is the same. "
        #     "Please check that --normalize is set or unset for "
        #     "both pre-training and here"
        # )
        if hasattr(cfg, "encoder_layers") and hasattr(w2v_args.model, "encoder_layers"):
             # Ensure the current config value takes precedence
             w2v_args.model.encoder_layers = cfg.encoder_layers
             logger.info(f"Overriding encoder_layers from checkpoint. Set to: {w2v_args.model.encoder_layers}")
        else:
            logger.info(f"No encoder_layers found in config or checkpoint. Using default value: {w2v_args.model.encoder_layers}")
