...
class AVHubertConfig(FairseqDataclass):
    ...
    encoder_layers: int = field(
        default=24, metadata={"help": "num encoder layers in the transformer"}
    )
    ...
    # CAR module
    car_enabled: bool = field(
        default=True, metadata={"help": "enable CAR module in conformer blocks"}
    )
    car_d_compress_ratio: int = field(
        default=4, metadata={"help": "compression ratio for CAR module (d_model / car_d_compress_ratio)"}
    )
    car_activation_fn: ChoiceEnum(utils.get_available_activation_fns()) = field(
        default="gelu", metadata={"help": "activation function to use in CAR module"}
    )
    car_dropout: float = field(
        default=0.1, metadata={"help": "dropout probability for the CAR module"}
    )
